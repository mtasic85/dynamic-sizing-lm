...
Tokens per second (original): 2.23
============================================================
Qwen3ForCausalLM(
  (model): Qwen3Model(
...
Resolving data files: 100%|█████████████████████████████████████████████████████████████| 1024/1024 [00:00<00:00, 45969.40it/s]
Resolving data files: 100%|█████████████████████████████████████████████████████████████| 1024/1024 [00:00<00:00, 48308.54it/s]
...
Computing norms...
Step 5/5: Applying pruning and converting to sparse
Applying pruning to 196 layers...
Pruning layer 1/196: model.layers.0.self_attn.q_proj
/home/mtasic/projects/dynamic-sizing-lm/demo_prune_3.py:196: UserWarning: Sparse CSR tensor support is in beta state. If you miss a functionality in the sparse tensor support, please submit a feature request to https://github.com/pytorch/pytorch/issues. (Triggered internally at /pytorch/aten/src/ATen/SparseCsrTensorImpl.cpp:53.)
  W = W.to_sparse_csr()
Pruning layer 2/196: model.layers.0.self_attn.k_proj
Pruning layer 3/196: model.layers.0.self_attn.v_proj
...
Pruning layer 9/196: model.layers.1.self_attn.k_proj
Pruning layer 10/196: model.layers.1.self_attn.v_proj
...
Pruning layer 21/196: model.layers.2.mlp.down_proj
Pruning layer 22/196: model.layers.3.self_attn.q_proj
...
Pruning layer 42/196: model.layers.5.mlp.down_proj
Pruning layer 43/196: model.layers.6.self_attn.q_proj
...
Pruning layer 49/196: model.layers.6.mlp.down_proj
Pruning layer 50/196: model.layers.7.self_attn.q_proj
...
Pruning layer 60/196: model.layers.8.self_attn.o_proj
Pruning layer 61/196: model.layers.8.mlp.gate_proj
Pruning layer 62/196: model.layers.8.mlp.up_proj
Pruning layer 63/196: model.layers.8.mlp.down_proj
Pruning layer 64/196: model.layers.9.self_attn.q_proj
...
Pruning layer 70/196: model.layers.9.mlp.down_proj
Pruning layer 71/196: model.layers.10.self_attn.q_proj
...
Traceback (most recent call last):
  File "/home/mtasic/projects/dynamic-sizing-lm/demo_prune_3.py", line 494, in <module>
    nonzero_params = count_nonzero_parameters(pruned)
  File "/home/mtasic/projects/dynamic-sizing-lm/demo_prune_3.py", line 420, in count_nonzero_parameters
    return sum((p.values() != 0).sum().item() if p.is_sparse else (p != 0).sum().item() for p in model.parameters())
  File "/home/mtasic/projects/dynamic-sizing-lm/demo_prune_3.py", line 420, in <genexpr>
    return sum((p.values() != 0).sum().item() if p.is_sparse else (p != 0).sum().item() for p in model.parameters())
                                                                   ^^^^^^
...
class SparseLinear(nn.Linear):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if self.weight.is_sparse:
            if self.weight.layout == torch.sparse_csr:
                out = torch.sparse.mm(self.weight, input.t()).t()
...
def apply_pruning(
    ...
    for name, mod in linears.items():
        ...
        if sparsity_type == "unstructured":
            ...
            N_str, M_str = sparsity_type.split(":")
            N, M = int(N_str), int(M_str)
            if M <= 1 or N >= M:
                raise ValueError("Invalid N:M, N < M >1")
            # Adjust sparsity_ratio to (M-N)/M
        ...
        # Convert to sparse CSR and replace module with SparseLinear
        W = W.to_sparse_csr()
        ...
        new_linear = SparseLinear(linear.in_features, linear.out_features, linear.bias is not None)
...
def prune_wanda(
    ...
    print("Step 5/5: Applying pruning and converting to sparse")
    apply_pruning(model, linears, norms, sparsity_ratio, sparsity_type)
...
def count_parameters(model: nn.Module) -> int:
    return sum(p.numel() if not p.is_sparse else p.values().numel() for p in model.parameters())
...
if __name__ == "__main__":
    ...
    # Prune using Wanda with structured N:M
    pruned = prune_wanda(
        model,
        tokenizer,
        sparsity_ratio=0.5,
        sparsity_type="2:4",  # Structured N:M
    ...
    nonzero_params = count_nonzero_parameters(pruned)
    pruned_params = total_params_orig - nonzero_params  # Use orig as sparse numel is nnz
    unchanged_params = nonzero_params

    print(f"Pruned model total parameters (non-zero): {nonzero_params}")
    ...
    # Save pruned model in .bin format (safetensors does not support sparse; use this for reduced size)
    save_dir = model_name.replace('/', '-') + "-pruned-" + str(nonzero_params)
    pruned.save_pretrained(save_dir, safe_serialization=False)
    tokenizer.save_pretrained(save_dir)
    print(f"Pruned model saved to: {save_dir} (in .bin format for sparse support)")
    ...
    if os.path.exists(bin_path):
        ...
        print(f"Saved pruned model file size: {file_size_mb:.2f} MB")
